...
from nets_tod import Net_tod, MNIST_Net_TOD, CIFAR10_Net_TOD
from query_strategies import RandomSampling, LeastConfidence, MarginSampling, EntropySampling, \
								...
								LossPredictionLoss, VAAL, WAAL, CyclicOutputDependecy
...
def get_net_TOD(name, args_task, device):
	if name == 'MNIST':
		return Net_tod(MNIST_Net_TOD, args_task, device)
	elif name == 'CIFAR10':
		return Net_tod(CIFAR10_Net_TOD, args_task, device)
	elif name == 'MNIST_pretrain':
		return Net_tod(MNIST_Net_TOD, args_task, device)
	elif name == 'FashionMNIST':
		return Net_tod(MNIST_Net_TOD, args_task, device)
		...
		return Net_tod(openml_Net_TOD, args_task, device)
	elif name == 'BreakHis':
		return Net_tod(CIFAR10_Net_TOD, args_task, device)
	elif name == 'PneumoniaMNIST':
		return Net_tod(PneumoniaMNIST_Net_TOD, args_task, device)
	elif name == 'waterbirds':
		return Net_tod(waterbirds_Net_TOD, args_task, device)
	elif name == 'waterbirds_pretrain':
		return Net_tod(waterbirds_Net_TOD, args_task, device)
	else:
		raise NotImplementedError
...
def get_strategy(STRATEGY_NAME, dataset, net, args_input, args_task):
	...
	elif STRATEGY_NAME == 'COD':
		return CyclicOutputDependecy(dataset, net, args_input, args_task)
	else:
		raise NotImplementedError
