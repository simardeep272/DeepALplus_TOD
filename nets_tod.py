...
class Net_tod:
    def __init__(self, net, params, device):
		# This is the implementation for the paper: 'Semi-Supervised Active Learning with Temporal Output Discrepancy'
		# TO DO: Add Auxiliary unsupervised loss for TOD implementation
        self.net = net
        self.params = params
        self.device = device
        self.cod_model = self.net().to(self.device)         # Initialize COD model
        self.detach_params()                                # Detach COD model parameters
    ...
    def train(self, data):
        ...
        self.clf.train()
        # self.cod = self.net(dim=dim, pretrained=self.params['pretrained'], num_classes=self.params['num_class']).to(self.device)
    ...
    def predict_uncertainty(self, unlabeled_data): 
		# Predict top uncertain samples from the unlabeled data
        self.clf.eval()
        self.cod_model.eval()
        ...
        with torch.no_grad():
            for inputs, labels, idxs in unlabeled_data_loader:
                ...
                out, el = self.clf(inputs)
                scores = F.softmax(out, dim = 1)
				
                cod_out,el = self.cod_model(inputs)
                cod_score = F.softmax(cod_out, dim=1)
                pred_loss = (scores - cod_score).pow(2).sum(1) / 2                     # Difference in features of COD model and Backbone
                uncertainty = torch.cat((uncertainty, pred_loss), dim=0)

        return uncertainty.to(self.device)
    ...
    def detach_params(self):
        for params in self.cod_model.parameters():
            params.detach_()
		
    def load_cod_weights(self, checkpoint):
        self.cod_model.load_state_dict(checkpoint)
		
        
class MNIST_Net_TOD(nn.Module):
	def __init__(self, dim = 28 * 28, pretrained=False, num_classes = 10):
		...
		self.dim = self.resnet18.fc.in_features
...
class CIFAR10_Net_TOD(nn.Module):
	def __init__(self, dim = 28 * 28, pretrained=False, num_classes = 10):
		super().__init__()
		self.resnet18 = models.resnet18(pretrained=pretrained)
		features_tmp = nn.Sequential(*list(self.resnet18.children())[:-1])
...
class openml_Net_TOD(nn.Module):
...
class PneumoniaMNIST_Net_TOD(nn.Module):
	def __init__(self, dim = 28 * 28, pretrained=False, num_classes = 10):
		super().__init__()
...
class waterbirds_Net_TOD(nn.Module):
	def __init__(self, dim = 28 * 28, pretrained=False, num_classes = 10):
		super().__init__()
		resnet18 = models.resnet18(pretrained=pretrained)
